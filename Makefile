...
yellowearth:
	apptainer run --nv --cleanenv --containall --writable-tmpfs \
		...
		"-filter_complex" \
		"bwdif=mode=0:parity=-1:deint=0,tvai_up=model=iris-3:scale=1:preblur=0:noise=0:details=0:halo=0:blur=0:compression=0:estimate=8:device=0:vram=1:instances=1,tvai_up=model=thd-3:scale=0:w=640:h=480:noise=0:blur=1:compression=0:prenoise=0.02:device=0:vram=1:instances=1,tvai_up=model=hyp-1:scale=1:w=640:h=480:parameters='sdr_ip=0.7\:hdr_ip_adjust=1\:saturate=0.3':device=0:vram=1:instances=1,setparams=range=pc:color_primaries=bt2020:color_trc=smpte2084:colorspace=bt2020nc,scale=w=640:h=480:flags=lanczos:threads=0:force_original_aspect_ratio=decrease,pad=640:480:-1:-1:color=black" "-c:v" "h264_videotoolbox" "-profile:v" "high" "-pix_fmt" "yuv420p" "-allow_sw" "1" "-g" "30" "-b:v" "0" "-q:v" "82" "-map" "0:a?" "-map_metadata:s:a:0" "0:s:a:0" "-map_metadata:s:a:1" "0:s:a:1" "-c:a" "copy" "-bsf:a:0" "aac_adtstoasc" "-map_metadata" "0" "-map_metadata:s:v" "0:s:v" "-fps_mode:v" "passthrough" "-movflags" "frag_keyframe+empty_moov+delay_moov+use_metadata_tags+write_colr" "-bf" "0" "-metadata" "videoai=Deinterlaced and enhanced using iris-3; mode: auto; revert compression at 0; recover details at 0; sharpen at 0; reduce noise at 0; dehalo at 0; anti-alias/deblur at 0; and focus fix Off. Enhanced using thd-3; revert compression at 0; sharpen at 100; reduce noise at 0; focus fix Off; and add noise at 2. HDR Enhanced using hyp-1; exposure at 1; saturation at 0.3; and highlight threshold at 0.7. Changed resolution to 640x480" "/movies/yellowearth_corrected.mp4"
